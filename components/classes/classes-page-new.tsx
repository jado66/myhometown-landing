...
export function ClassesPageClient({ crcs }: ClassesPageClientProps) {
  ...
  // Get real classes data for the selected CRC
  const {
    classes: realClasses,
    loading,
    error,
  } = useClasses({
    communityId: selectedCRC?.community?.id,
    autoFetch: !!selectedCRC?.community?.id,
  });
  ...
  useEffect(() => {
    if (availableClasses.length <= 1 || isPaused || selectedCRC) {
      if (isPaused) {
  ...
  }, [availableClasses.length, isPaused, selectedCRC]);
  ...
  return (
    <Container>
      <div className="min-h-screen bg-background">
        ...
        {/* Advanced Class Registration Interface */}
        {selectedCRC && (
                          ...
                          <div className="space-y-6">
                            {isClassFull(selectedClass) && (
                              <Alert>
                                <AlertCircle className="h-4 w-4" />
                                <AlertDescription>
                                  This class is currently full. Contact the CRC
                                  to check for waitlist availability.
                                </AlertDescription>
                              </Alert>
                            )}
